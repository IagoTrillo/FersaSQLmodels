-- Este análisis son las cosas sinsentido de Julien.....
-- se decide qué referencias se analiza comparando el stock en cada filial a nivel de catalog con las ventas a mas de 10 años a nivel de catalog....
-- Y luego, para esas referencias, se calcula los stocks a tirar con la diferencia entre el stock y las ventas a 4 años pero solo para esas referencias para las que hay stock para mas de 10....
...
to_allocate AS (
  SELECT
    r.filial,
    r.catalog,
    r.stock_tras_consumo_4y AS stock_to_allocate
  FROM resumen r
LEFT JOIN leveltop_catalog ltc
...
item_caps AS (
  SELECT
    ...
    COALESCE(sei.qty_4y_item, 0) AS qty_4y_item,
    GREATEST(oi.stockQ_item - COALESCE(sei.qty_4y_item,0), 0) AS max_cap_item
  FROM obso_items oi
...
SELECT
  ...
  COALESCE(sei.qty_4y_item, 0) 					AS "External sales last 4y for this item in this subsidiary",
  coalesce(sec.qty_4y,0)						AS "External sales last 4y for this catalog globally",
  coalesce(sec.qty_10y,0)						AS "External sales last 10y for this catalog globally",
...
left join sales_exe_agg sec
	on sec.catalog=rf.catalog
ORDER BY
  rf.filial,
  rf.catalog,
